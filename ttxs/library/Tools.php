...
class Tools {
    ...
    public static function byteTrans($size, $digits){
    ...
    public static function authcode($string,$operation = 'D'){ 
        $key = md5(self::AUTH_KEY); 
     ...
     *@return string
     */
    public static function intToString($str){
        $array = array(
                'P','T','G','a','k','q','r','W','B','h'
                );
        $arr = str_split($str);
    ...
    // 获取 年 月 周 信息
    public static function getYmw($timestamp = 0){
        if(empty((int)$timestamp))
            $timestamp = time();

        $ymw = date('Y-m-d-W', (int)$timestamp);
        $temArr = explode('-', $ymw);
        $temArr[] = $temArr[0];
        
        $temArr = array_map(function($v){
            return (int)$v;
        }, $temArr);

        // 1月前三天
        if($temArr[1] == 1 && in_array($temArr[2], [1,2,3]) && $temArr[3] != 1){
            $temArr[0] -= 1;
        }
        
        // 12 月最后三天
        if($temArr[1] == 12 && in_array($temArr[2], [29,30,31]) && $temArr[3] == 1){
            $temArr[0] += 1;
        }
