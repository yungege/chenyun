...
class Verify {
    ...
    public function __construct($config=array()){
        ...
        self::$redis    =   Cache_CacheRedis::getInstance();
        $this->_uname   = $uname;
    }
    ...
    public function check($code, $id = '') {
        //$key = self::PREFIX . $this->authcode($this->seKey . session_id());
        $key = self::PREFIX . session_id();
        // 验证码为空或者过期
        $secode = self::$redis->get($key);
    ...
    /**
     * 输出验证码并把验证码的值保存的Redis中
     * 验证码保存到Redis的格式为： array('verify_code' => '验证码值', 'verify_time' => '验证码创建时间');
     * @access public     
    ...
    public function entry($id = '') {
        ...
        // 保存验证码
        // $key        =   $this->authcode($this->seKey . session_id());
        $key        =   session_id();
        $code       =   $this->authcode(strtoupper(implode('', $code)));
        $secode     =   array();
        $secode['verify_code'] = $code; // 把校验码保存到session
        $secode['verify_time'] = time();  // 验证码创建时间
        self::$redis->set(self::PREFIX.$key, $secode, $this->expire);
