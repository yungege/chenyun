...
abstract class Db_Db {
	...
	public function __construct() {
        $params = Yaf_Registry::get('database')[0];
	 ...
	 * @return bool
	 */
	public function insert($table, $data, $null_values = false, $use_cache = true, $type = Db_Db::INSERT) {
		if (!$data && !$null_values)
			return true;
	...
	public function delete($table, $where = '', $limit = 0, $use_cache = true) {
		...
		if ($use_cache && $this->is_cache_enabled)
			Cache_Cache::getInstance()->deleteQuery($sql);
	...
	public function executeS($sql, $array = true, $use_cache = true) {
		...
		$this->result = false;
		$this->last_query = $sql;
		if ($use_cache && $this->is_cache_enabled && $array && ($result = Cache_Cache::getInstance()->get(md5($sql))))
		{
		...
		if ($use_cache && $this->is_cache_enabled)
			Cache_Cache::getInstance()->setQuery($sql, $result_array);
	...
	public function getRow($sql, $use_cache = true) {
		...
		if ($use_cache && $this->is_cache_enabled && ($result = Cache_Cache::getInstance()->get(md5($sql))))
		{
			$this->last_cached = true;
		...
		if ($use_cache && $this->is_cache_enabled)
			Cache_Cache::getInstance()->setQuery($sql, $result);
	...
	public function numRows() {
		if (!$this->last_cached && $this->result)
		{
			...
			if ($this->is_cache_enabled)
				Cache_Cache::getInstance()->set(md5($this->last_query) . '_nrows', $nrows);

			return $nrows;
		}
		else if ($this->is_cache_enabled && $this->last_cached)
			return Cache_Cache::getInstance()->get(md5($this->last_query) . '_nrows');
	...
	protected function logError($sql = false) {
		...
		{
			Log::writeLog('sql_error', 'E', $this->getMsgError() . ':' . $sql);
		}
	}
